...
//!
//! kqueue is measured directly here, so this only builds where kqueue exists.
//! The numbers in the module below were taken on macOS.
...
fn main() {
    eprintln!("floor_probe waits on kqueue directly, so it only runs on a BSD");
}
